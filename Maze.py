...
class Maze:
    wallBlock, wayBlock = OBSIDIAN, STONE
    ...
    def __init__(self, width, height):
        ...
        for y in range(self.height):
            ...
            for x in range(self.width):
                if y % 2 + x % 2 < 2:
                    row.append(Maze.wallBlock)
                else:
                    row.append(Maze.wayBlock)
    ...
    def _addWalls(self, y, x):
        for o in Maze.offset:
            ...
            if self._range(cell[0], cell[1]) and cell not in self.visited and cell not in self.frontier and \
                    self.maze[cell[0]][cell[1]] == Maze.wayBlock:
    ...
    def workOneStep(self):
        ...
        if len(self.frontier) > 0:
            ...
            if dy != 0:
                self.maze[inmaze[0] + dy][inmaze[1]] = Maze.wayBlock
                self.toDraw.add((inmaze[0] + dy, inmaze[1]))
            if dx != 0:
                self.maze[inmaze[0]][inmaze[1] + dx] = Maze.wayBlock
